    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                if !isFirstRun {
                    HStack { Spacer(); CircleButton(systemImage: "xmark", label: "Schließen") { dismiss() } }
                }
                VStack(alignment: .leading, spacing: 10) {
                    Image(systemName: "icloud.and.arrow.up")
                        .font(.system(size: 26, weight: .semibold))
                        .frame(width: 58, height: 58)
                        .background(Color.brandYellow, in: RoundedRectangle(cornerRadius: 18, style: .continuous))
                    ...
                    Text("Mit Konto sind deine Gutscheine gesichert und auf iPhone und im Browser gleich. Fotos und PINs bleiben nur auf deinem Gerät.")
                ...
                VStack(spacing: 8) {
                    ...
                    box(mode == .register ? "Passwort (mind. 8 Zeichen)" : "Passwort") {
                        SecureField("••••••••", text: $password)
                            ...
                            .focused($focus, equals: .password).submitLabel(.go).onSubmit { Task { await submit() } }
                    }
                }
                ...
                if isFirstRun {
                    ...
                    Text("Du kannst später jederzeit in den Einstellungen ein Konto anlegen.")
                        .font(.system(size: 13)).foregroundStyle(Color.muted).frame(maxWidth: .infinity)
                        .multilineTextAlignment(.center)
                }
                Link("Datenschutz", destination: APIConfig.baseURL.appendingPathComponent("datenschutz"))
